...
//
// s3_inspect — the S3 (state augmentation / stochastic cloning) stage inspector
// (epic #371, issue #378). The real-data companion to the synthetic S3 probe
// (s3_augmentation.cpp); renders on the filter-internal tier, built on the S4
// inspector template (s4_inspect.cpp).
//
// It runs the real estimator (VioEstimator + MsckfBackend) over a real EuRoC
// sequence until the filter holds a genuinely-correlated covariance (initialized,
// with a few clones in the window), then takes a COPY of that keyframe state and
// runs the real `StateHelper::augment_clone` on it (via branes/tools/
// s3_augmentation_inspect.hpp). It records the before/after covariance and the
// stochastic-cloning residuals — the clone's marginal and cross-covariance vs the
// cloned pose, and whether P stays PSD. The figure (docs-site/scripts/
// gen-augmentation-figures.mjs) draws the before/after heatmap with the new clone
// block outlined.
//
// No SDK change: augment_clone is already standalone and backend().state() is
// exposed, so the inspector clones a copy of the live state.
//
// Usage:
//   s3_inspect --dataset <EuRoC mav0 dir> [--out <dir>] [--min-clones N] [--max-frames N]
//
//   --dataset     mav0 directory (cam0/, imu0/). Required.
//   --out         output dir (default build/stage_probes/S3)
//   --min-clones  capture once the window holds this many clones (default 5)
//   --max-frames  give up waiting after this many frames
//
// Then render:
//   node docs-site/scripts/gen-augmentation-figures.mjs <out>
//
// EuRoC (~1.5 GB) is not vendored, so this is a developer tool; the inspector
// logic is gated by tests/tools/s3_augmentation_inspect.cpp.
...
struct Args {
    std::string dataset;
    std::string out = "build/stage_probes/S3";
...
std::uint64_t to_u64(const std::string& raw) {
    if (!raw.empty() && (raw.front() == '-' || raw.front() == '+'))
        throw std::runtime_error("s3_inspect: expected a non-negative integer, got '" + raw + "'");
    std::size_t pos = 0;
    ...
    if (raw.empty() || pos != raw.size())
        throw std::runtime_error("s3_inspect: invalid integer value '" + raw + "'");
    return v;
}
...
Args parse(int argc, char** argv) {
    ...
    auto next = [&](int& i) -> std::string {
        if (i + 1 >= argc)
            throw std::runtime_error("s3_inspect: missing value after " + std::string(argv[i]));
        return argv[++i];
    };
    for (int i = 1; i < argc; ++i) {
        ...
        if (v == "--help" || v == "-h")
            ...
            throw std::runtime_error("s3_inspect: unknown argument '" + std::string(v) + "'");
    }
    return a;
}

void usage() {
    std::cout << "s3_inspect — S3 augmentation inspector (real EuRoC keyframe → cloned covariance)\n\n"
                 "  s3_inspect --dataset <EuRoC mav0 dir> [--out <dir>] [--min-clones N] [--max-frames N]\n\n"
                 "  Then: node docs-site/scripts/gen-augmentation-figures.mjs <out>\n";
}
...
int main(int argc, char** argv) {
    ...
    if (args.dataset.empty()) {
        std::cerr << "s3_inspect: --dataset <EuRoC mav0 dir> is required.\n\n";
        usage();
        return 2;
    }
    ...
    try {
        ...
        std::cerr << "s3_inspect: " << ex.what() << "\n";
        return 1;
    }
    if (images.empty() || imu.empty()) {
        std::cerr << "s3_inspect: empty EuRoC streams (" << images.size() << " images, " << imu.size() << " IMU)\n";
        return 1;
    }
    ...
    // Replay until the filter holds a correlated window (initialized + min_clones).
    std::uint64_t processed = 0;
    std::size_t imu_idx = 0;
    bool captured = false;
    bt::S3AugmentRecord record;
    for (const auto& frame : images) {
        ...
        if (est.backend().initialized() && est.backend().state().clones.size() >= args.min_clones) {
            bt::S3AugmentationInspector::S3Input in{est.backend().state()};  // copy the live state
            record = bt::S3AugmentationInspector().run(in);
            captured = true;
            break;
        }
    }
    ...
    if (!captured) {
        std::cerr << "s3_inspect: did not reach " << args.min_clones << " clones within " << processed
    ...
    const std::string out_path = args.out + "/augmentation.json";
    std::ofstream os(out_path);
    if (!os) {
        std::cerr << "s3_inspect: cannot write " << out_path << "\n";
        return 1;
    }
    os << bt::to_json(record).dump(1) << '\n';
    if (!os) {
        std::cerr << "s3_inspect: error writing " << out_path << "\n";
        return 1;
    }

    std::cout << "s3_inspect: captured at " << processed << " frames, " << record.n_clones_after - 1
              << " clones in window\n"
              << "  augment:  dim " << record.dim_before << " → " << record.dim_after << " (+6 clone block at offset "
              << record.clone_offset << ")\n"
              << "  contract: clone marginal err " << record.clone_marginal_err << ", cross err "
              << record.clone_cross_err << ", PSD " << (record.psd ? "yes" : "NO") << "\n"
              << "  out:      " << out_path << "\n"
              << "  render:   node docs-site/scripts/gen-augmentation-figures.mjs " << args.out << "\n";
    return 0;
}
